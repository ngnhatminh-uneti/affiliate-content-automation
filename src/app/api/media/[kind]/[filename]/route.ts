...
export async function GET(_request:Request,context:{params:Promise<{kind:string;filename:string}>}){
  ...
  const extension=path.extname(filename).slice(1).toLowerCase();const contentType=types[extension];
  if(!contentType)return NextResponse.json({error:"Định dạng media không được hỗ trợ"},{status:415});
  const root=path.resolve(getMediaDir(kind));const file=path.resolve(root,filename);
  if(!file.startsWith(`${root}${path.sep}`))return NextResponse.json({error:"Đường dẫn không hợp lệ"},{status:400});
  try{
    const data=await fs.readFile(file);
    return new Response(new Uint8Array(data),{headers:{"Content-Type":contentType,"Cache-Control":"no-store"}});
  }catch{return NextResponse.json({error:"Không tìm thấy media"},{status:404});}
}
